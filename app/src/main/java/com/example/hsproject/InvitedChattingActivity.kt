...
import com.example.hsproject.databinding.ActivityInvitedChattingBinding
...
class InvitedChattingActivity : BaseActivity() {

    lateinit var binding : ActivityInvitedChattingBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this ,R.layout.activity_invited_chatting)
        setValues()
        setupEvents()
    }
...
//            맵 구조 활용 한번에 서버에 전송
            ...
            database.getReference("${invitedAppointmentData.id}message").setValue(inputMap)

            binding.contentEdt.setText("")
        }
        //항목의 데이터 변경 감지
        database.getReference("${invitedAppointmentData.id}message")
